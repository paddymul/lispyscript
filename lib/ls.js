...
templates["for_"] = _.template("for(; (<%= loop_condition %>);) {  \n <%= loop_body %>; }" );
...
//templates["for_"] = _.template("for(; (<%= loop_condition %>);) {  \n <%= loop_body %>; }" );
keywords["for_"] = function(arr) {
    if (arr.length < 2) handleError(0, arr._line, arr._filename);
    if (typeof arr[1] != "object") handleError(0, arr._line);
    return templates["for_"]({
        loop_condition: handleExpressions(arr[1]),
        loop_body: handleExpressions(arr.slice(2)),
